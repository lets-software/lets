...
/*		CSS Files Called by script:																								*/
		if (!$print) {
			$styles 								.= 		" @import url(".URL.'templates/'.TEMPLATE."/styles/noticeboard_page.css);\n";
	...
	if ($mysql->result('SELECT * FROM noticeboard WHERE noticeboardID = '.$_GET['page_id'].' LIMIT 1')) {
		...
		if (ENABLE_INSTANT_BUY and $notice['quick_delete'] and isset($_POST['buy_now_amount']) and user_type() and !$_SESSION['member_suspended'] and $_SESSION['member_validated']) {
			...
			if (LOCK_BUY_NOW_PRICE and $_POST['buy_now_amount'] != $notice['amount']) {
				$messages .= $messages_indent.'<span class="message">You must pay '.$notice['amount'].' for this item</span><br /><br />'."\n";
			} elseif (!is_numeric($_POST['buy_now_amount']) or $_POST['buy_now_amount'] < 1) {
				$messages .= $messages_indent.'<span class="message">You must enter a positive number</span><br /><br />'."\n";
			} else {
				if (NEGATIVE_BALANCE_LIMIT != '0.00') {
					if ($transactions->balance($_SESSION['member_id'])) {
						if (($transactions->balance - $amount) < (NEGATIVE_BALANCE_LIMIT * -1)) {
							...
							$messages .= $messages_indent.'<span class="message">A limit of '.NEGATIVE_BALANCE_LIMIT.' '.CURRENCY_NAME.' has been established. This trade cannot be completed because it would exceed that limit.</span><br /><br />'."\n";
						}		
					}
				}
		...
		$noticeboard_info .= $noticeboard_info_indent.' <span class="nb_page_stats">'.ucwords(NOTICEBOARD_NAME_SINGULAR).' ID: '.$notice['noticeboardID']."</span><br />\n";
		$noticeboard_info .= $noticeboard_info_indent.' <span class="nb_page_stats">Created '.return_month($notice['created_month']).' '.$notice['created_day'].', '.$notice['created_year'].' by ';
		...
		if ($notice['expiry_year']) {
			if (!$expired) {
				$noticeboard_info .= $noticeboard_info_indent.' <span class="nb_page_stats">Expires '.return_month($notice['expiry_month']).' '.$notice['expiry_day'].', '.$notice['expiry_year']."</span><br />\n";
			} else {
				$noticeboard_info .= $noticeboard_info_indent.' <span class="nb_page_stats">Expired '.return_month($notice['expiry_month']).' '.$notice['expiry_day'].', '.$notice['expiry_year']."</span><br />\n";
			}
		}
		$noticeboard_info .= $noticeboard_info_indent.' <br /><span class="nb_page_type">';
		if ($notice['type'] == 2) {
			$noticeboard_info .= 'Auction';
		}
		if ($notice['item'] == 1 and $notice['type'] != 2) {
			$noticeboard_info .= " Item";
			if ($notice['request']) {
				$noticeboard_info .= " Requested";
			} else {
				$noticeboard_info .= " Offered";
			}
		}
		if ($notice['item'] == 0 and $notice['type'] != 2) {
			$noticeboard_info .= " Service";
			if ($notice['request']) {
				$noticeboard_info .= " Requested";
			} else {
				$noticeboard_info .= " Offered";
		...
		if ($noticeboard_owner_suspended) {
			$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">The owner of this ".strtolower(NOTICEBOARD_NAME_SINGULAR)." has been suspended<br />".ucfirst(MEMBERS_NAME_PLURAL)." will not be able to access it until ".$user->full_name($notice['accountID'])."'s account is re-activated</span><br /><br />\n";
		}
		if ($expired and $bought) {
			$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">This auction has ended</span><br /><br />\n";
		} elseif ($expired) {
			$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">This ".strtolower(NOTICEBOARD_NAME_SINGULAR)." has expired</span><br /><br />\n";
		} elseif ($bought) {
			if (user_type() == 0 or $_SESSION["member_validated"] == 0 or $_SESSION["member_suspended"] == 1) {
				$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">This item has been purchased</span><br /><br />\n";
			} elseif (user_type() == 1 or user_type() == 2) {
				if ($mysql->result('SELECT buyerID FROM transactions WHERE noticeboardID = '.$notice['noticeboardID'].' LIMIT 1')) {
					if ($mysql->result['buyerID'] == $_SESSION["member_id"]) {
						$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">You have purchased this item</span><br /><br />\n";
					} else {
						$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">".$user->full_name($mysql->result['buyerID'])." purchased this item</span><br /><br />\n";
					}
				} else {
					$noticeboard_info .= $noticeboard_info_indent." <span class=\"nb_page_expired\">This item has been purchased</span><br /><br />\n";
				}
		...
		if ($notice['amount']) {
			$noticeboard_body .= "\n".$noticeboard_body_indent." <span class=\"nb_page_title\">Amount: ".$notice['amount']."</span><br /><br />\n";
		} else {
			$noticeboard_body .= "<br />\n";
		}
		if ($thumb_html) {
