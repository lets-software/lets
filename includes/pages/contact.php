...
if (!user_type() and bad_user()) {
    $message .= $i.T_('<span class="message">You aren\'t allowed to send any more emails through our system. 
                                    Please use your own email solution.<br /><br />
                                    <strong>If you are not able to properly see the email addresses on this website you need to enable javascript.</strong></span>')."<br /><br />\n"; // TODO: eMail address should never be display anyway, form should be used at all time.
    ...
    if (isset($_POST['email_address']) and isset($_POST['email_subject']) and isset($_POST['email_message'])) {
        if ($_POST['email_address'] and $_POST['email_subject'] and $_POST['email_message']) {
            if (!user_type()) {
                wrong_password(); // TODO: Check if this is this the right function?
            }
            ...
            if (filter_var($_POST['email_address'], FILTER_VALIDATE_EMAIL)) {
                ...
                $message .= $i.T_('<li class="error">Please enter a valid email address.</li>')."<br /><br />\n";
            }
            if (!$mail_error) {
                $message .= $i.T_('<li class="ok">Your message has been successfully sent.</li>')."<br /><br />\n";
                if (ENABLE_LOG) log_action('Someone attempted to contact '.SITE_NAME.'.');
            } else {
                $message .= $i.T_('<li class="error">Sorry your email could not be sent.</li>')."<br /><br />\n";
                //echo $mail_error;
                log_error('Failed to send an email from contact.php');
            }
        } else {
            $message .= $i.T_('<span class="message">All fields are required.</span>')."<br /><br />\n";
        }
    } else {
...
if ($submitted_encrypted_email) {
    ...
    $message .= $i.T_(' <strong>Please submit the following form</strong> to contact: ').$display_email."\n";
    $message .= $i." <fieldset>\n";
    $message .= $i.' <form name="contact" method="post" action="'.URL.$url.append_url($url).'">'."\n";
    $message .= $i.T_('  <label for="email_address">Your email Address:</label>')."\n";
    $message .= $i.'  <input type="text" id="email_address" name="email_address" /><br />'."\n";
    $message .= $i.T_('  <label for="email_subject">Subject:</label>')."\n";
    $message .= $i.'  <input type="text" id="email_subject" name="email_subject" /><br />'."\n";
    $message .= $i.T_('  <label for="email_message">Message:</label>')."\n";
    $message .= $i.'  <textarea id="email_message" name="email_message"></textarea><br /><br />'."\n";
    ...
    $message .= $i.T_('  <input class="email_button" type="submit" name="submit" value="Submit" />')."\n";
    $message .= $i.' </form>'."\n";
    $message .= $i." </fieldset>\n";
    $message .= $i.'</div>'."\n";
    $message .= $i."<!-- /contact_form -->\n";
