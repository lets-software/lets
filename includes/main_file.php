	...
	if (count($left_bracket_array) > 1) {
		...
		foreach ($left_bracket_array as $key => $lb_text) {
			...
			if ($b_tag_check != $tag_only_array[0] && ($right_bracket_array[1] || $ending_tag_array[1])) {
				$HTML_report .= "<li><strong>[".$tag_only_array[0]."]</strong> must be lower case.</li>";         	
			...
			if ($b_tag_check == 'a' && strtolower(substr($text, 0, 1)) != 'a') {
				...
				if (count($quo_check) != 3) {
					$HTML_report .= "<li>Check your <strong>[a]</strong> tags. <em>TIP: Possible href missing quotes and/or misplaced spaces</em></li>"; 
				} else if ($tag_only_array[2] || substr($tag_only_array[1], 0, 5) != 'href=') {
					$HTML_report .= "<li>Only the href attribute (in lowercase) is available for <strong>[a]</strong> tags</li>"; 
				}
			}
			...
			if($ending_tag_array[1]){
				...
				if(($ending_tag_array[1] != $begin_nesting_store[$Count_BNS]) || (!$ending_tag_array[1] && $begin_nesting_store[$Count_BNS]) || ($ending_tag_array[1] && !$begin_nesting_store[$Count_BNS])){
					if($ending_tag_array[1] && $begin_nesting_store[$Count_BNS]){
						 $HTML_report .= "<li>Check your <strong>[".$ending_tag_array[1]."]</strong> tags and your <strong>[".$begin_nesting_store[$Count_BNS]."]</strong> tags. <em>TIP: Possible Improper Nesting or Missing Tag</em></li>";
					}
					if($ending_tag_array[1] && !$begin_nesting_store[$Count_BNS]){
						 $HTML_report .= "<li>Check your <strong>[".$ending_tag_array[1]."]</strong> tags. <em>TIP: Possible Non-Beginning Tag</em></li>";
					}
					if(!$ending_tag_array[1] && $begin_nesting_store[$Count_BNS]){
						 $HTML_report .= "<li>Check your <strong>[".$begin_nesting_store[$Count_BNS]."]</strong> tags. <em>TIP: Possible Non-Ending Tag</em></li>";
					}
				}
		...
		for($e = 0; $e <= $Count_BNS2; $e++){
			 ...
			 if($last_BNS_check && !is_null($last_BNS_check)){
				 $HTML_report .= "<li>Check your <strong>[".$last_BNS_check."]</strong> tags. <em>TIP: Possible Non-Ending Tag</em></li>";
			 }
		}
		if ($HTML_report) {
			$HTML_report = "Invalid Code » Please check for the following:\n\n<div class=\"indent\"><ul>".$HTML_report."</ul></div>";
		}
		return $HTML_report;
	} else {
		return null;
